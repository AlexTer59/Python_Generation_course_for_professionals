...
К Тимуру приходят n покупателей, называют номер класса, за который они хотят приобрести книгу, и сумму, которую они
...
Вторая строка содержит число n — количество покупателей. В последующих n строках вводятся два числа, разделенные
пробелом, где первое число является номером класса, второе — суммой, предлагаемой покупателем.
...
for _ in range(int(input())):
    ...
    total_earned += bool(book in books) * price
