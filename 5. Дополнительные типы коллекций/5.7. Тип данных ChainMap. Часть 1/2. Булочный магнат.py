...
for k, v in order.items():
    total_price += ingredients[k] * v
    print(f'{k:{len(max(order.keys(), key=len)) + 1}}x {v}')
    max_str_len.append(len(f'{k:{len(max(order.keys(), key=len)) + 1}}x {v}'))
print('-' * max(*max_str_len, len(f'ИТОГ: {total_price}р')))
