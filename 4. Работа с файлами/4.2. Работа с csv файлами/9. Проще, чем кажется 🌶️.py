...
def condense_csv(filename, id_name):
    with open(filename, 'r', encoding='UTF-8') as file:
        objects_dict = {}
        for obj, attr, val in csv.reader(file):
            if obj not in objects_dict:
                objects_dict[obj] = {id_name: obj}
            objects_dict[obj][attr] = val
    with open('condensed.csv', 'w', encoding='utf-8', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=objects_dict[obj])
        writer.writeheader()
        writer.writerows(objects_dict.values())
