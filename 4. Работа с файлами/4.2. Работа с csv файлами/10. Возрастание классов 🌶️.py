...
за период
2000 — 2021 г. В первом столбце записан год, в последующих столбцах записан класс и количество учеников в данном
классе в этом году:
...
with open('10. student_counts.csv', 'r', encoding='UTF-8') as file:
    reader = csv.DictReader(file)
    headers = sorted(reader.fieldnames, key=lambda x: (int(x.split('-')[0]), x[-1]) if x[0].isdigit() else ())
    rows = list(reader)
with open('10. sorted_student_counts.csv', 'w', encoding='UTF-8') as file:
